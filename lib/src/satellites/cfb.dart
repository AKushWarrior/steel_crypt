...
class CfbSatellite {
  ...
  CfbSatellite(this.key, this.padding);
  ...
  String encrypt({@required String inp, @required String iv}) {
    ...
    var cipher = (padding == PaddingAES.none)
        ? CFBBlockCipher(AESFastEngine(), 64)
        : PaddedBlockCipher('AES/CFB-64/' + parsePadding(padding));
...
class CfbSatelliteRaw {
  Uint8List key;
  PaddingAES padding;

  CfbSatelliteRaw(this.key, this.padding);
